...
void main( void )
{
  ...
  // io_initialize
  for( i=0; i<nIO_COUNT; i++ ) {
    IOPinConfig( tagRelayOutput[i], MODE_OUTPUT );
  }
  ...
  // message parsing
    ...
    if( GetMessage(&rxmsg) ) {
      if( rxmsg.nMsgType == PACKET_HEADER ) {
        ...
        for( i=0; i<8; i++ ) { tmpPacket.nData[i] = 0; }
        
        switch( rxmsg.nMsgSubType & 0xff00 ) {
          case tagCAN_OUTPUT_RELAY  :
            if( !(rxmsg.nData[0] & rxmsg.nData[1]) ) {
              ResetMaskRelayOutput( rxmsg.nData[1] );
              SetMaskRelayOutput( rxmsg.nData[0] );
              tmpPacket.nData[0] = GetRelayOutput();
